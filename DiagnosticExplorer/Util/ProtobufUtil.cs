...
public static class ProtobufUtil
{
    public static byte[] Compress<T>(T obj, int compressThreshold)
    {
        ...
        using (
            GZipStream gstream = new(compressedStream, CompressionMode.Compress, leaveOpen: true)
        )
        {
            stream.Position = 1;
    ...
    public static T Decompress<T>(byte[] body)
    {
        if (body == null || body.Length < 1)
        {
            throw new ArgumentException(
                "Decompress requires a non-empty payload with a leading mode byte.",
                nameof(body)
            );
        }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            if (_read > _maxBytes)
            {
                throw new InvalidDataException(
                    $"Decompressed payload exceeds the {_maxBytes} byte limit."
                );
            }
